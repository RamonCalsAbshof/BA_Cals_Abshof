...
class OverTimeClustering(object):
    ...
    def __init__(self, data, min_cf=0.015, sw=3):
        """
        Params:
        ...
        self._similarities = []
        self._adaptabilities = []
        self._connection_factors = []
        self._temporal_connection_factors = []

    def get_factor_df(self, factor_type, **kwargs):
        """
        Params:
    ...
    def get_temporal_connection_factor(self):
        """
        Returns:
    ...
    def get_temporal_connection_factor_sw(self, sw=3):
        """
        Returns:
            temporal_connection_factors (array) - array with shape (num_timestamps, num_objects, num_objects) containing
                                                  the temporal connection factors for all object pairs at all timestamps
    ...
    def get_clusters(self, min_cf, sw=3):
        """
        Params:
    ...
    def get_clusters_with_noise(self, min_cf, sw=3):
        """
        Params:
            min_cf (float) - threshold for the minimum connection factor for inserting edges to the graph
        Optional:
    ...
    def get_clusters_df(self, min_cf=0.5, sw=3):
        """
        Optional:
    ...
    def get_noisy_clusters_df(self, min_cf=0.5, sw=3):
        """
        Optional:
            min_cf (float) - threshold for the minimum connection factor for inserting edges to the graph (default:0.5)
            sw (int) - width of sliding window (default: 3)
    ...
    def create_factors_df(self, factors):
        """
        Params:
    ...
    def calc_similarity(self, feature_vectors):
        """
        Params:
            feature_vectors (array) - array with shape (num_timestamps, num_objects, num_features) containing the
    ...
    @staticmethod
    def calc_adaptability(similarities):
        """
        Params:
    ...
    def calc_connection_factor(self, similarities):
        """
        Params:
            similarities (array) - array with shape (num_timestamps, num_objects, num_objects) containing the
                                  similarities of all objects at all timestamps
    ...
    def calc_temporal_connection(self, connection_factors):
        """
        Params:
            connection_factors (array) - array with shape (num_timestamps, num_objects, num_objects) containing the
    ...
    def calc_temporal_connection_sw(self, connection_factors, sw=3):
        """
        Params:
            connection_factors (array) - array with shape (num_timestamps, num_objects, num_objects) containing the
                                  connection factors of all objects at all timestamps
    ...
    @staticmethod
    def calc_cluster_peers_indices(factors, min_cf):
        """
        Params:
    ...
    @staticmethod
    def calc_cluster_peers(factors, min_cf):
        """
        Params:
            factors (array) - array with shape (num_timestamps, num_objects, num_objects) containing the factors
                                  for all object pairs at all timestamps.
    ...
    def calc_clusters(self, factors, min_cf):
        """
        Params:
            factors (array) - array with shape (num_timestamps, num_objects, num_objects) containing the factors
                                  for all object pairs at all timestamps.
    ...
    #########################################################################
    #################### Graph Illustration Functions #######################
    #########################################################################

    def create_nx_graphs(self, factors, min_cf=0.5):
        """
        Params:
            factors (array) - array with shape (num_timestamps, num_objects, num_objects) containing the factors
                                  for all object pairs at all timestamps.
        ...
        for i in range(len(self._timestamps)):
            G = nx.DiGraph()
            G.add_nodes_from(self._object_ids)
            for oid in range(len(self._object_ids)):
                for oid2 in range(len(self._object_ids)):
                    if factors[i, oid, oid2] >= min_cf:
                        G.add_weighted_edges_from([(self._object_ids[oid], self._object_ids[oid2], factors[i, oid, oid2])])
        return graphs

    def create_nx_factors_graphs(self, factor_type, min_cf=0.5, sw=3):
        """
        Params:
            factor_type (str) - 'similarity', 'adaptability', 'connection', 'temporal_connection' or
                                'temporal_connection_sw' indicating the factor type to calculate
        ...
        Returns
            graphs (list) - list with length num_timestamps containing the graphs (networkx.Graph) for each timestamp
        """
        if factor_type == 'temporal_connection_sw':
            factors = self.get_factors(factor_type, sw=sw)
        else:
            factors = self.get_factors(factor_type)

        graphs = self.create_nx_graphs(factors, min_cf)
        return graphs

    @staticmethod
    def save_nx_graph_fig(graph, path):
        """
        Params:
            graph (networkx.Graph) - networkx Graph representing the data
            path (str) - path where to save the Graph image
        """
        nx.draw(graph)
        plt.savefig(path)
        return
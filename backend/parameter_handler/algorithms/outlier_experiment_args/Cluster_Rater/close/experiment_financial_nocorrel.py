...
for document in db.find():
    ...
    if document['ric'] != 'nan':
        for year in range(1998,2017,1):
            try:
                if ((document[str(year)]['TR-TotalDebtOutstanding'] != "nan") and (document[str(year)]['TR-TotalDebtOutstanding'] != '') and (document[str(year)]['TR-TotalDebtOutstanding'] != '0')
                    and (document[str(year)]['TR-NetIncome'] != "nan") and (document[str(year)]['TR-NetIncome'] != '') and (document[str(year)]['TR-NetIncome'] != '0')
                    and (document[str(year)]['TR-TtlLiabShareholderEqty'] != "nan") and (document[str(year)]['TR-TtlLiabShareholderEqty'] != '') and (document[str(year)]['TR-TtlLiabShareholderEqty'] != '0')
                    and (document[str(year)]['TR-TotalAssetsReported'] != "nan") and (document[str(year)]['TR-TotalAssetsReported'] != '') and (document[str(year)]['TR-TotalAssetsReported'] != '0')):
                        ric = document['ric']
                        feature_RoA = float(document[str(year)]['TR-NetIncome']) / float(document[str(year)]['TR-TotalAssetsReported'])
                        feature_Leverage = float(document[str(year)]['TR-TtlLiabShareholderEqty']) / float(document[str(year)]['TR-TotalDebtOutstanding'])
                        series_list.append(pd.Series([ric, str(year), feature_RoA, feature_Leverage], index=['ObjectID', 'Time', 'RoA', 'Leverage']))
            except:
                print('NetIcome: ' + document[str(year)]['TR-NetIncome'] + ' TotalAssets: ' +document[str(year)]['TR-TotalAssetsReported'] +
                      ' ShareholderEqty: ' + document[str(year)]['TR-TtlLiabShareholderEqty'] + ' TotalDebt: ' + document[str(year)]['TR-TotalDebtOutstanding'])


t_data = pd.DataFrame(series_list, columns=['ObjectID', 'Time', 'RoA', 'Leverage'])
...
def plotTimePoints(data, no, title='uknown parameter'):
    ...
    def f(x, y, z, w, **kwargs):
        ...
        for i in range(len(x)):
            ax.annotate(str(i), xy=(x.values[i], y.values[i]), fontsize=10,
    ...
    g.map(f, "RoA", "Leverage", "ObjectID", "cluster", alpha=0.6, s=5, legend='full')
...
def normalize(data):
    Distance = data[['RoA']].values.astype(float)
    arrdelay = data[['Leverage']].values.astype(float)
...
def start_KMEANS_experiment():
    ...
    for k in range(1, 20, 1):
        ...
        columns = ['ObjectID', 'Time', 'cluster', 'RoA', 'Leverage']
